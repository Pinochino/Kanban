...
const MyProjectTasks = () => {
  ...
  const projectRequestUrl = projectId ? `${apiName.projects.detail}/${projectId}` : apiName.projects.list;

  const {
    data: projectData,
    isLoading,
    isError,
  } = useGetAllData({ url: projectRequestUrl, enabled: Boolean(projectId) });
      ...
      <Card className="border-0 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 text-white shadow-xl">
        <CardContent className="flex flex-col gap-3 p-6">
          ...
          <p className="max-w-2xl text-sm text-slate-200">
            {projectId
              ? `${t("taskBoard.projectTasksTitle")}: ${projectId}`
